...
const Home = () => {
  ...
  const getMovieData = async (page = 1) => {
    try {
      const { data } = await axios.get(`https://omdbapi.com/?apikey=392901a4&s=${searchValue}&page=${page}`);
      if (data?.Response === "True") {
        setMovieAllData((prevData) => page === 1 ? data?.Search : [...prevData, ...data?.Search]);
        setTotalResults(parseInt(data?.totalResults, 10)); // Store the total results from the API response
      ...
      <div className='grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-6 gap-6'>
        {isDataAvailable ? (
          movieAllData.map((movie) => (
            <div className='bg-white border border-gray-200 rounded-lg shadow-md overflow-hidden' key={movie.imdbID}>
          ...
          <h1 className='col-span-full text-center text-2xl text-gray-500'>No results found</h1>
        )}
      </div>
